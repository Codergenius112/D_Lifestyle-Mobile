...
// All valid BookingStatus values from the backend enum
...
export default function BookingsScreen({ navigation }: any) {
  ...
  const fetchBookings = useCallback(async (isRefresh = false) => {
    try {
      ...
      // GET /bookings — user derived from JWT; no userId needed in URL
  ...
  const handleCardPress = (booking: BackendBooking) => {
    switch (booking.bookingType) {
      case 'ticket':
        ...
        break;
      case 'table':
        navigation.navigate('Bookings', { bookingId: booking.id });
        break;
      case 'apartment':
        // Navigate to booking detail view (ApartmentBooking shows existing booking by bookingId)
        navigation.navigate('ApartmentBooking', { bookingId: booking.id });
        break;
      case 'car':
        // CarDetailsScreen expects { rentalId }, not { bookingId }
      ...
      <TouchableOpacity key={booking.id} style={styles.bookingCard}
        ...
        <View style={styles.bookingInfo}>
          ...
          <Text style={styles.bookingResource} numberOfLines={1}>
            Resource: {booking.resourceId?.slice(0, 12)}...
          </Text>
...
const styles = StyleSheet.create({
  ...
  bookingCard: {
    ...
    borderRadius: 12, marginBottom: 12, overflow: 'hidden', padding: 16,
  },
  ...
  groupBadge: {
    ...
    backgroundColor: 'rgba(255,152,0,0.15)', paddingHorizontal: 6,
  ...
  exploreButton: {
    backgroundColor: '#f5dd4b', paddingHorizontal: 32,
